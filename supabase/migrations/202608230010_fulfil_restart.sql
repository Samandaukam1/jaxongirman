/**
 * Fulfilment learns the difference between renewing and restarting.
 *
 * `order_fulfil` is redefined whole because PL/pgSQL has no way to replace one
 * branch of a function. Everything outside the subscription branch is the
 * definition from 202608160006, unchanged.
 */
...
begin
  ...
  case v_order.purpose
    ...
    when 'subscription' then
      ...
      /**
       * A restart turns the cycle over instead of extending it.
       *
       * Applied here rather than from a button, which is what makes both halves
       * of the promise true at once: the money arriving and the cycle turning
       * over are one transaction, and a payment that fails leaves the old
       * tariff exactly as it was, because none of this runs until it arrives.
       */
      if coalesce((v_order.metadata ->> 'restart')::boolean, false) then
        select * into v_subscription from public.user_subscriptions
         where id = public.apply_subscription_restart(v_order.user_id, v_plan.id, v_order.id);

        -- The notification below serves both paths; only its title differs.
      else
      ...
      v_result := jsonb_build_object(
        ...
        'restarted', coalesce((v_order.metadata ->> 'restart')::boolean, false));

      insert into public.notifications (user_id, kind, title, body, payload, deep_link)
      values (
        v_order.user_id, 'order_paid',
        case when coalesce((v_order.metadata ->> 'restart')::boolean, false)
             then 'Tarif qayta boshlandi' else 'Tarif faollashtirildi' end,
