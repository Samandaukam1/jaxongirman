-- Which refusal a candidate is given when two apply.
--
-- Claiming a reward ends the ladder, so somebody who has claimed and then taps
-- a higher rung was told "you have not reached it yet" — true of the counts and
-- beside the point: the rung is closed to them at any count. The prior claim is
-- now checked first, which is also the order the two rules matter in.
