-- Fulfilment learns to hand over votes.
--
-- A vote order must not fall through to the product branch below it: that path
-- settles a `marketplace_purchases` row against a product this order does not
-- have. The branch is explicit, and everything it does — the ledger rows, the
-- escrow release, both notifications — happens inside one function so no path
-- can write half of it.
--
-- The rest of the function is unchanged from `202608110013`.
...
begin
  ...
  case v_order.purpose
    ...
    -- ------------------------------------------------------ marketplace --
    -- ------------------------------------------------------ marathon votes --
    when 'marathon_votes' then
      -- The transfer, the escrow release and both anonymous notifications live
      -- in one function so that no path can write half of them.
      v_result := public.marathon_fulfil_vote_sale(v_order.id);
