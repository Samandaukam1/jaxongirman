 ...
 *
 * A copy of `web/lib/env-guard.ts` and `admin/src/lib/env-guard.ts`. The three
...
const SETUP_HINT = "Copy user/.env.example to user/.env and set it, then restart the bundler — EXPO_PUBLIC_* values are inlined at build time.";
...
export function publishableKey(key: string | undefined, variable: string): string {
  ...
  if (looksSecret(key)) {
    throw new Error(
      `${variable} holds a service-role or secret key. That key must never ship inside an app — rotate it in the Supabase dashboard and set the publishable (anon) key here instead.`,
    );
  }
